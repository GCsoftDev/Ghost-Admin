...
export default Component.extend({
    ...
    actions: {
        ...
        async replaceWithCardSection(cardName, range, payload, label) {
            let editor = this.editor;
            let {head: {section}} = range;

            const response = await this.ajax.request(`${ghostPaths().apiRoot}/gamalon-content/${label}`, {
                processData: false,
                contentType: false
            });
            payload = {html: response.contentData};
